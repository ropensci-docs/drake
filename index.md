...
![infographic](https://docs.ropensci.org/drake/reference/figures/infographic.svg)

| Usage | Release | Development |
|:---|:---|:---|
| [![Licence](https://img.shields.io/badge/licence-GPL--3-blue.svg)](https://www.gnu.org/licenses/gpl-3.0.en.html) | [![CRAN](https://www.r-pkg.org/badges/version/drake)](https://cran.r-project.org/package=drake) | [![check](https://github.com/ropensci/drake/workflows/check/badge.svg)](https://github.com/ropensci/drake/actions?query=workflow%3Acheck) |
| [![minimal R version](https://img.shields.io/badge/R%3E%3D-3.3.0-blue.svg)](https://cran.r-project.org/) | [![cran-checks](https://cranchecks.info/badges/summary/drake)](https://cran.r-project.org/web/checks/check_results_drake.html) | [![lint](https://github.com/ropensci/drake/workflows/lint/badge.svg)](https://github.com/ropensci/drake/actions?query=workflow%3Alint) |
| [![](https://tinyverse.netlify.com/badge/drake)](https://CRAN.R-project.org/package=drake) | [![rOpenSci](https://badges.ropensci.org/156_status.svg)](https://github.com/ropensci/software-review/issues/156) |  |
| [![downloads](https://cranlogs.r-pkg.org/badges/drake)](https://CRAN.R-project.org/package=drake) | [![JOSS](https://joss.theoj.org/papers/10.21105/joss.00550/status.svg)](https://doi.org/10.21105/joss.00550) | [![](https://bestpractices.coreinfrastructure.org/projects/2135/badge)](https://bestpractices.coreinfrastructure.org/projects/2135) |
|  | [![Zenodo](https://zenodo.org/badge/82609103.svg)](https://zenodo.org/badge/latestdoi/82609103) | [![superseded lifecycle](https://img.shields.io/badge/lifecycle-superseded-blue.svg)](https://lifecycle.r-lib.org/articles/stages.html) |

  

As of 2021-01-21, `drake` is
[superseded](https://lifecycle.r-lib.org/articles/stages.html). The
[`targets`](https://docs.ropensci.org/targets/) R package is the
long-term successor of `drake`, and it is more robust and easier to use.
Please visit <https://books.ropensci.org/targets/drake.html> for full
context and advice on transitioning.
...
[![workflowing](https://docs.ropensci.org/drake/reference/figures/workflowing.png)](https://www.youtube.com/embed/jU1Zv21GvT4)
...
[![commcall](https://docs.ropensci.org/drake/reference/figures/commcall.png)](https://ropensci.org/commcalls/2019-09-24/)
...
So far, we have just been setting the stage. Use
...
![hist1](https://docs.ropensci.org/drake/reference/figures/graph.png)

The next [`make()`](https://docs.ropensci.org/drake/reference/make.md)
just builds `hist` and `report.html`. No point in wasting time on the
data or model.
...
literate programming with [knitr](https://yihui.org/knitr/), and version
control with
...
- The `quiet` column appears above because one of the
...
- Advanced: if you use `make(cache_log_file = TRUE)` and put the cache
  ...
  with the `git` commit history of your code.
...
![](reference/figures/unnamed-chunk-18-1.png)
...
this point, you relied on basic `drake` functions such as
[`make()`](https://docs.ropensci.org/drake/reference/make.md), so you
may not have needed to peek at any substantive author-defined code in
advance. In that case, you can stay usefully ignorant as you reimplement
the original author’s methodology. In other words, `drake` could
potentially improve the integrity of independent replication.
...
- `drake` takes care of the parallel scheduling and high-performance
  computing (HPC) for you. That means the HPC code is no longer tangled
  up with the code that actually expresses your ideas.
...
- [`pleurosoriopsis`](https://github.com/joelnitta/pleurosoriopsis): The
  workflow that supports [Ebihara *et al.* 2019. “Growth Dynamics of the
  Independent Gametophytes of *Pleurorosiopsis makinoi* (Polypodiaceae)”
...
- [`make()`](https://docs.ropensci.org/drake/reference/make.md): build
  your project.
- [`drake_history()`](https://docs.ropensci.org/drake/reference/drake_history.md):
  show what you built, when you built it, and the function arguments you
  used.
- [`r_make()`](https://docs.ropensci.org/drake/reference/r_make.md):
  launch a fresh [`callr::r()`](https://github.com/r-lib/callr) process
  to build your project. Called from an interactive R session,
  [`r_make()`](https://docs.ropensci.org/drake/reference/r_make.md) is
  more reproducible than
  [`make()`](https://docs.ropensci.org/drake/reference/make.md).
- [`loadd()`](https://docs.ropensci.org/drake/reference/readd.md): load
  one or more built targets into your R session.
- [`readd()`](https://docs.ropensci.org/drake/reference/readd.md): read
  and return a built target.
...
- [`outdated()`](https://docs.ropensci.org/drake/reference/outdated.md):
  see which targets will be built in the next
  [`make()`](https://docs.ropensci.org/drake/reference/make.md).
- [`deps_code()`](https://docs.ropensci.org/drake/reference/deps_code.md):
  check the dependencies of a command or function.
- [`drake_failed()`](https://docs.ropensci.org/drake/reference/drake_failed.md):
  list the targets that failed to build in the last
  [`make()`](https://docs.ropensci.org/drake/reference/make.md).
- [`diagnose()`](https://docs.ropensci.org/drake/reference/diagnose.md):
  return the full context of a build, including errors, warnings, and
  messages.
...
The workshop at `https://github.com/wlandau/learndrake` devotes
particular attention to `drake`’s mental model.

- The [user manual](https://books.ropensci.org/drake/)
- [`drakeplanner`](https://github.com/wlandau/drakeplanner), an R/Shiny
  app to help learn `drake` and create new projects. Run locally with
  `drakeplanner::drakeplanner()` or access it at
  <https://wlandau.shinyapps.io/drakeplanner>.
- \``https://github.com/wlandau/learndrake`, an R package for teaching
  an extended `drake` workshop. It contains notebooks, slides, Shiny
  apps, the latter two of which are publicly deployed. See
  `https://github.com/wlandau/learndrake/blob/main/README.md` for
  instructions and links.

## In practice

- [Miles McBain](https://github.com/MilesMcBain)’s [excellent blog
  post](https://milesmcbain.xyz/the-drake-post/) explains the motivating
  factors and practical issues {drake} solves for most projects, how to
  set up a project as quickly and painlessly as possible, and how to
  overcome common obstacles.
- Miles’ [`dflow`](https://github.com/MilesMcBain/dflow) package
  generates the file structure for a boilerplate `drake` project. It is
  a more thorough alternative to
  [`drake::use_drake()`](https://docs.ropensci.org/drake/reference/use_drake.md).
- `drake` is heavily function-oriented by design, and Miles’
  [`fnmate`](https://github.com/MilesMcBain/fnmate) package
  automatically generates boilerplate code and docstrings for functions
  you mention in `drake` plans.

## Reference

- The [reference website](https://docs.ropensci.org/drake/).
- The [official repository of example
  code](https://github.com/wlandau/drake-examples). Download an example
  workflow from here with
...
- Presentations and workshops by [Will
  ...
  Stawitz](https://github.com/cstawitz), and others. See specific links
  at <https://books.ropensci.org/drake/index.html#presentations>
- The [FAQ page](https://books.ropensci.org/drake/faq.html), which links
  to [appropriately-labeled issues on
...
- [Other known issues](https://github.com/ropensci/drake/issues) (please
  search both open and closed ones).
...
- Broman, Karl W (2012). “Halotype Probabilities in Advanced Intercross
  Populations.” *G3* 2(2), 199-202.Powered by the `Makefile` at
...
- Instead of a
  ...
  language](https://books.ropensci.org/drake/plans.html#large-plans) for
  declaring targets.
- Targets in [GNU Make](https://www.gnu.org/software/make/) are files,
  whereas targets in `drake` are arbitrary variables in memory. (`drake`
  does have opt-in support for files via
  ...
  `drake` caches these objects in its own [storage
  system](https://github.com/richfitz/storr) so R users rarely have to
...
of a complex data analysis pipeline. [knitr](https://yihui.org/knitr/)’s
job is to communicate those expensive results after `drake` computes
them. [knitr](https://yihui.org/knitr/) / [R
...
plan](https://books.ropensci.org/drake/plans.html), and use
[`loadd()`](https://docs.ropensci.org/drake/reference/readd.md) and
[`readd()`](https://docs.ropensci.org/drake/reference/readd.md) to refer
to targets in the report itself. See an [example
...
However, data poses a challenge. The datasets created by
[`make()`](https://docs.ropensci.org/drake/reference/make.md) can get
large and numerous, and it is not recommended to put the `.drake/` cache
or the `.drake_history/` logs under version control. Instead, it is
recommended to use a data storage solution such as DropBox or
...
The `workflowr` package is a project manager that focuses on literate
programming, sharing over the web, file organization, and version
control. Its brand of reproducibility is all about transparency,
communication, and discoverability. For an example of `workflowr` and
`drake` working together, see [this machine learning
...
[graduate school](https://www.stat.iastate.edu/), for first introducing
me to the idea of [Makefiles](https://www.gnu.org/software/make/) for
...
rOpenSci](https://github.com/ropensci/software-review/issues/156), and
to [Maëlle Salmon](https://github.com/maelle) for such active
involvement as the editor. Thanks also to the following people for
contributing early in development.
